...
@testset "cluster! with initial representatives" begin
    ...
    @testset "Test4: cols_to_groupby by default ([:year]) and cols_to_crossby = [:scenario]" begin
        @testset "Test4.1: initial representatives for all years" begin
            connection = _new_connection_multi_scenario_year(;
                profile_names,
                num_timesteps,
                years,
            ...
            # define the layout with cols_to_crossby = [:scenario]
            ...
            unique_mapping_combinations =
                unique(df_rep_periods_mapping[:, [:year, :scenario]])
            ...
            # but should include year column (it's in cols_to_groupby by default)
            df_profiles_rep_periods =
                DuckDB.query(
                    connection,
                    "FROM profiles_rep_periods
                    ORDER BY year, profile_name, rep_period, timestep",
                ) |> DataFrame
            ...
            expected_rows =
                length(years) * length(profile_names) * num_rps * period_duration
            @test nrow(df_profiles_rep_periods) == expected_rows
            ...
            for year in years
                for scenario in scenarios
                    for profile in profile_names
                        ...
                        # Find representative periods for this combination
                        matching_rows = df_profiles_rep_periods[
                            (df_profiles_rep_periods.year .== year) .& (df_profiles_rep_periods.profile_name .== profile),
                            :,
                        ]
                        @test nrow(matching_rows) > 0
                        # Check if any representative period has values close to our initial representatives
        ...
        @testset "Test4.2: initial representatives for only one year" begin
            connection = _new_connection_multi_scenario_year(;
                profile_names,
                num_timesteps,
                years,
            ...
            # Check that initial representatives are only used for 2020
            # Note: scenarios are NOT multiplied here since scenario column is not in profiles_rep_periods
            df_profiles_rep_periods =
                DuckDB.query(
                    connection,
            ...
            # Should have data for 2020 not depending on the number of scenarios: profiles × num_rps × period_duration
            ...
            for scenario in scenarios
                for profile in profile_names
                    ...
                    # Find representative periods for this combination in 2020
                    matching_rows = df_profiles_rep_periods[
                        (df_profiles_rep_periods.profile_name .== profile),
                        :,
                    ]
                    @test nrow(matching_rows) > 0
                    # Check if any representative period has values close to our initial representatives
...
@testset "cluster! with drop_incomplete_last_period and cols_to_crossby" begin
    period_duration = 4
    num_timesteps = 10  # 2 complete periods + 1 incomplete (2 timesteps)
    num_rps = 2
    years = [2020]
    scenarios = [1, 2]
    profile_names = ["name1"]
    ...
    # Each scenario should have only 2 complete periods (incomplete period dropped)
    num_complete_periods = 2
    expected_mapping_rows = length(years) * length(scenarios) * num_complete_periods
    @test nrow(df_rep_periods_mapping) == expected_mapping_rows

    # The maximum period per year-scenario should equal num_complete_periods
    grouped = groupby(df_rep_periods_mapping, [:year, :scenario])
    max_periods = combine(grouped, :period => maximum => :max_period)
